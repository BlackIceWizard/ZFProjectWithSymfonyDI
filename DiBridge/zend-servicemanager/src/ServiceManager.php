...
use RiverRing\DiPoc\DiBridge\Exception\CrossLibraryServiceNotFound;
...
class ServiceManager implements ServiceLocatorInterface
{
     ...
     * @return object|array
     * @throws CrossLibraryServiceNotFound
     * @throws Exception\ServiceNotFoundException
     */
    public function get($name, $usePeeringServiceManagers = true)
    {
        try {
            return $this->originalGet($name, $usePeeringServiceManagers);
        } catch (Exception\ServiceNotFoundException $zendException) {
            try {
                if($this->symfonyContainerBuilder === null) {
                    $this->symfonyContainerBuilder = $this->originalGet(SymfonyContainerBuilderProvider::class)->containerBuilder();
                }

                return $this->symfonyContainerBuilder->get($name);
            } catch(\Exception $symfonyException) {
                throw new CrossLibraryServiceNotFound($zendException, $symfonyException);
            }
        }
    }

    private function originalGet($name, $usePeeringServiceManagers = true)
    {
